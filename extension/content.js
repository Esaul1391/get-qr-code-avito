...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  // Сбор отзывов (RAW, без парсинга)
  if (msg?.type === "COLLECT_REVIEWS") {
    try {
      const result = collectReviewsRawSync();
...
// ------------------------------------------------------------
// 3) ОТЗЫВЫ: RAW-COLLECTOR (без парсинга)
// ------------------------------------------------------------
function collectReviewsRawSync() {
  // 1) Стабильный путь: берём ТОЛЬКО корневые отзывы review(n) без /header /body
  const reviewRoots = Array.from(
    document.querySelectorAll('[data-marker^="review("]:not([data-marker*="/"])')
  );

  if (reviewRoots.length > 0) {
    const reviews = reviewRoots.map((root) => {
      const hashUserId = root.closest("[data-hash-user-id]")?.getAttribute("data-hash-user-id") || null;
      const marker = root.getAttribute("data-marker") || null;
      return {
        html: root.outerHTML,
  ...
  // 2) Fallback: твой старый heuristic (можно оставить, но лучше чуть подкрутить root-выбор)
  const candidates = new Set();

  for (const el of document.querySelectorAll("[data-marker]")) {
    const m = (el.getAttribute("data-marker") || "").toLowerCase();
    if (m.includes("feedback") || m.includes("review") || m.includes("comment") || m.includes("rating")) {
      candidates.add(el);
    }
  }

  for (const el of document.querySelectorAll(
    'article, li, [class*="feedback"], [class*="review"], [class*="comment"], [class*="rating"]'
  )) {
    const cls = (el.className || "").toString().toLowerCase();
    if (cls.includes("feedback") || cls.includes("review") || cls.includes("comment") || cls.includes("rating")) {
      candidates.add(el);
    }
  }
  ...
  const looksLikeReviewText = (t) => {
    t = (t || "").toLowerCase();
    return t.includes("сделка") || t.includes("отзыв") || t.includes("покупатель") || t.includes("продавец") || t.includes("оценк");
  };

  for (const el of candidates) {
    // Пытаемся подняться именно до review(n) (если где-то выше он есть)
    const reviewRoot =
      el.closest('[data-marker^="review("]:not([data-marker*="/"])') ||
      el.closest('[itemtype="https://schema.org/Review"]') ||
      el.closest("article, li") ||
      el;

    const key = reviewRoot.getAttribute("data-marker") || reviewRoot.outerHTML.slice(0, 200);
    if (seenKeys.has(key)) continue;

    const t = (reviewRoot.innerText || "").trim();
